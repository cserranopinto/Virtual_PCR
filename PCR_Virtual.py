...
try:
    ...
    print("""
Librería Biopython no está instalada.
Si usa conda, pruebe creando ambiente:
conda create -n pcr conda-forge::biopython

Creado el ambiente, acceder usando:
conda activate pcr
""")
    exit()
...
parser.add_argument('-r', '--Reverse', help = 'Secuencia reverse (mismo hebra y sentido que forward)', type=str)
...
def PCR(ref, seq):
    ...
    """
    Esta función toma como argumento secuencias en formato str
    :param ref: string
    :param seq: str
    :return: list [coord1, coord2, secuencia]
    """
...
##REVISAR QUE VERSION DE BIOPYTHON FUNCIONA MEJOR, O CAMBIAR A OBJETO SEQ EN FUNCION
...
###############################################################################
        ...
        forward = PCR(fasta.seq.reverse_complement(), args.Forward)
        reverse = PCR(fasta.seq.reverse_complement(), args.Reverse)
